...
class AddAcc extends React.Component {
	...
	addAccount() {
		...
		let mob_num =
			document.querySelector('#add_acc_inp_mobnum').value + document.querySelector('#add_acc_inp_mob').value;

		let phone_num =
			document.querySelector('#add_acc_inp_phonenum').value + document.querySelector('#add_acc_inp_phone').value;
				...
				<div className="add_acc_con">
					...
					<div id="bottom_btns" className="two_items">
						...
						<button
							...
							onClick={
								this.state.loading ? null : this.state.mode === 'view' ? this.EditOn : this.addAccount
							}
						>
							{this.state.loading ? (
