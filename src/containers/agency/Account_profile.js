...
export default class Account_pro extends React.Component {
	getDet = async () => {
		...
		if (!start_date && !end_date && mode.value) {
			...
			let edate = n + "-04-01";
			await fetch(
				`/api/vouch/specific/${this.state.account.id}?sdate=${sdate}&edate=${edate}&mode=${mode.value}`
			)
				.then((res) => res.json())
				.then((data) => {
					if (data) {
		...
		if (start_date && end_date) {
			await fetch(
				`/api/vouch/specific/${this.state.account.id}?sdate=${start_date}&edate=${end_date}&mode=${mode.value}`
			)
				.then((res) => res.json())
				.then((data) => {
					if (data) {
	...
	clearall = () => {
		...
		fetch(
			`/api/vouch/specific/${this.state.account.acc_name}?sdate=${sdate}&edate=${edate}`
		)
			.then((res) => res.json())
			.then((data) => {
				if (data) {
	...
	setOpenEditModal = (type, open) => {
		type === "basic"
			? this.setState({ openBasicEditModal: open })
			: this.setState({ openBankingEditModal: open });
	};
	constructor(props) {
		super(props);
			...
			<div>
				...
				{this.state.account && (
					<div className="acc_pro_body">
							...
							<div className="acc_pro_right">
								...
								<div className="acc_pro_right_lower">
									...
									<div>
										...
										this.state.account.Bank_Details !== " " ? (
											JSON.parse(this.state.account.Bank_Details).map(
												(detail) => {
													return (
														<div style={{ borderTop: "1px solid #767676" }}>
															<div
																key={Math.random()}
																className="acc_pro_detail_last_lowr"
															>
																{detail.Bank_Acc_Num ? (
																	<div
																		className="acc_pro_detail_heading"
						...
						{this.state.val === "reports" && (
							<Report_pro acc_name={this.state.account.acc_name} />
						)}
					</div>
				)}
			</div>
