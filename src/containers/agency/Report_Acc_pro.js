...
import graph_icon from "assets/icons/graph_view.svg";
import menu from "assets/icons/grid.svg";
import blue_menu from "assets/icons/blue_grid.svg";
import blue_graph from "assets/icons/blue_graph.svg";
...
export default class Report_pro extends React.Component {
    ...
    if (ans === "this_month") {
      ...
      await this.setState(() => {
        return {
          ...
          data_sales: [
            0,
            this.Total_sales_WW("1"),
            this.Total_sales_WW("2"),
            this.Total_sales_WW("3"),
            this.Total_sales_WW("4"),
            this.Total_sales_WW("5"),
            this.Total_sales_WW("6")
          ],
          purchase: [
            0,
            this.Total_purchase_WW("1"),
            this.Total_purchase_WW("2"),
            this.Total_purchase_WW("3"),
            this.Total_purchase_WW("4"),
            this.Total_purchase_WW("5"),
            this.Total_purchase_WW("6")
          ],
          payment: [
            0,
            this.Total_payment_WW("1"),
            this.Total_payment_WW("2"),
            this.Total_payment_WW("3"),
            this.Total_payment_WW("4"),
            this.Total_payment_WW("5"),
            this.Total_payment_WW("6")
          ]
        };
      });
    }
  ...
  Sales_MW = ans => {
    ...
    this.state.monthdata.map((e, i) => {
      i == ans &&
        e.vouch.map(x => {
          x.supplier === this.props.acc_name && (t = parseInt(t) + parseInt(x.totalAmt));
        });
    });
    return t;
  };
  ...
  Purchase_MW = ans => {
    ...
    this.state.monthdata.map((e, i) => {
      i == ans &&
        e.vouch.map(x => {
          x.customer === this.props.acc_name && (t = parseInt(t) + parseInt(x.totalAmt));
        });
    });
    return t;
  };
  ...
  Payment_MW = ans => {
    ...
    this.state.monthdata.map((e, i) => {
      i == ans &&
        e.jovouch.map(x => {
          x.credit_acc === this.props.acc_name && (t = parseInt(t) + parseInt(x.amount - x.balance));
        });
    });
    return t;
  };
  ...
  Sales_DW = i => {
    ...
    this.state.dayData.map(e => {
      e.vouch.map(x => {
        x.bill_date === finaldate && x.supplier === this.props.acc_name && (t = parseInt(t) + parseInt(x.totalAmt));
      });
    });
  ...
  Purchase_DW = i => {
    ...
    this.state.dayData.map(e => {
      e.vouch.map(x => {
        x.bill_date === finaldate && x.customer === this.props.acc_name && (t = parseInt(t) + parseInt(x.totalAmt));
      });
    });
