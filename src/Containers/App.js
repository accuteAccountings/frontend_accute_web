...
data = data.toLowerCase()
		if (this.state.ProOrAcc === 'Products') {
			let fPro = this.state.tempProducts.filter((pro) => {
				if (data === '') {
					return true;
				} else if (pro.product_name.toLowerCase().indexOf(data) === -1 && pro.hsn_num.toLowerCase().indexOf(data)=== -1 ) {
					return false;
				} else {
					return true;
				}
		...
		if (this.state.ProOrAcc === 'Accounts') {
			let fPro = this.state.tempAcc.filter((acc) => {
				if (data === '') {
					return true;
				} else if (acc.acc_name.toLowerCase().indexOf(data) === -1 && acc.print_name.toLowerCase().indexOf(data)=== -1 ) {
					return false;
				} else {
					return true;
				}
